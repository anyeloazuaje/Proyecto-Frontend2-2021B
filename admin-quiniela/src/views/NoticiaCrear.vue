      ...
      <v-container>
        <v-row class="flex align-center justify-space-between">
          <v-col class="justify-end" cols="12 text-center" sm="4">
            <v-btn @click="$router.go(-1)" color="info darken-1 " medium>
              <v-icon dark medium> mdi-arrow-left-thick </v-icon>
            </v-btn>
          </v-col>
                  ...
                  <v-card class="padding">
                    ...
                    >
                      ...
                      <v-textarea
                        ...
                        counter
                        append-icon="mdi-newspaper-variant-outline"
                        auto-grow
                        rows="4"
                        :loading="deshabilitado"
                      ...
                      <v-btn
                        ...
                        color="teal"
                      >
                        Crear noticia <v-icon>mdi-earth-plus</v-icon>
                      </v-btn>
                    </v-form>
                  </v-card>
                </v-col>
...
export default {
  ...
  methods: {
    async obtenerDatosSelect() {
      await Promise.all([this.obtenerAutores(), this.obtenerCategorias()]);
    },
    async CrearNoticia() {
      if (
      ...
      const noticiaACrear = JSON.parse(JSON.stringify(this.noticia));
      noticiaACrear.id_autor = autorSeleccionado.id;
      noticiaACrear.id_categoria = categoriaSeleccionada.id;
      try {
        this.deshabilitado = true;
        const { data } = await this.axios.post("/crear-noticia", noticiaACrear);
